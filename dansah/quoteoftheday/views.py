...
class QuoteOfTheDayView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        if not quote_of_the_day:
            return Response(
                {"status": "No quote of the day available"},
                status=status.HTTP_404_NOT_FOUND,
            )
        serializer = self.serializer_class(quote_of_the_day, many=True)
        return Response({"status": "success", "quote_of_the_day": serializer.data})
    ...
    def get(self, request, pk):
        ...
        if quote_of_day is None:
            return Response(
                {
                    "status": "fail",
                    "message": f"Quote of the day with Id: {pk} not found",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
...
class QuoteOfTheDayDetailDateView(generics.GenericAPIView):
    serializer_class = QuoteOfTheDaySerializer
    queryset = QuoteOfTheDay.objects.all()

    def get_quote_of_day_by_date(self, date):
        try:
            return QuoteOfTheDay.objects.get(quote_date=date)
        except:
            return None

    def get(self, request):
        date = request.query_params.get("day", None)
        quote_of_day = self.get_quote_of_day_by_date(date)
        if quote_of_day is None:
            return Response(
                {
                    "status": "fail",
                    "message": f"Quote of the day with data: {date} not found",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        serializer = self.serializer_class(quote_of_day)
